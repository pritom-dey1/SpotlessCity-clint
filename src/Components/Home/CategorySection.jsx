  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-16">
      <h2 className="text-3xl md:text-4xl uppercase font-bold text-center mb-10">
        Report By <span className="text-[#18ae50]">Category
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        {categories.map((cat, index) => (
          <div
            key={index}
            className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 flex flex-col items-center text-center hover:scale-105 transition-transform"
          >
            <div className="text-5xl mb-4">{cat.emoji}</div>
            <h3 className="text-xl font-semibold mb-2">{cat.name}</h3>
            <p className="text-gray-600 dark:text-gray-300 text-sm">{cat.description}</p>
          </div>
        ))}
      </div>
    </div>
